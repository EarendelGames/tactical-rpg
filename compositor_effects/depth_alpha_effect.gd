...
@export var depth_multiplier := 1.0;

func _init() -> void:
	var shader_file: RDShaderFile = load("res://compositor_effects/depth_alpha.glsl")
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	var parameters := PackedFloat32Array([size.x, size.y, depth_multiplier, 0.0])
